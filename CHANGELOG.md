...
- **Phase 19 documentation** — mdBook coverage in both languages: a "Discovery with `karpal`" guide (provider protocol, all nine tools with real outputs, index-compat) and a comprehensive Discovery Runtime reference (architecture, library API, dogfooding, drift gates), each with a Japanese counterpart; the legacy `karpal-index` guide carries a successor banner. `karpal-discovery`'s README is restructured (tool table, quickstart, architecture); the ROADMAP Phase 19 status is current; a consolidated phase design record lands in `docs/plans/`.
